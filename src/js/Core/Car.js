...
Car.prototype = {
		move : function(x,y){
			if(this.player){
				if(this.line_change){
					if( Math.abs( this.next_x - this.realx ) > 3 ){
			...
			if( this.front == null || ( this.leader && this.back == null )){
				if( this.line_change ){
					...
					if(this.back_crash > 5){
						...
						for( var car in other_cars ){
							if(  Math.floor(Math.abs( checky - other_cars[car].y )) <= checkr * 3 &&  Math.floor(Math.abs( checkx - other_cars[car].x )) <= checkr * 3 ){
								crash = true;
		...
		navigationMinusSpeedx : function( other_cars, back_cars, up_road, down_road ){
			...
			if( this.leader && this.back != null && this.back.player ){
				this.speedx = 0;
				this.speedy = 0;
				this.move(0,0);
				...
				if( this.line_change ){
					...
					var front_check = false;
					for( var car in other_cars ){
						if(other_cars[car].leader){
							if(( Math.floor(Math.abs( checky - other_cars[car].y )) <= checkr * 6 &&  Math.floor(Math.abs( checkx - other_cars[car].x )) <= checkr * 3 ) && other_cars[car].line_change ){
								this.speedy = 0;
								front_check = true;
								break;
							}
		...
		finish : function(){
			if( this.y <= 0 || this.y > 730 || this.x < 0 || this.x > 1200 ){
				return true;
			}else{
				return false;
			}
		...
		addPlayerMember : function( member, count ){

		},
		addMember : function( member, count ){
			if( member.player ){
				if( this.leader ){
					member.line_change = true;
					member.next_x = member.x + member.radius * 4;
					member.speedx = 0.5;
					this.speedy = 0;
					this.back = member;
					this.num_member++;
